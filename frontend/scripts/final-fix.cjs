...
// Let's just write the replacement logic reading from generated json. Oh wait, I'll put the generate code inside again.
const subjectsMeta = {
  science: {
    name: 'Science',
...
function genClassData(subjectSlug, classNum) {
  const classSlug = `class-${classNum}`;
  const topics = syllabusTopics[subjectSlug][classSlug];
  ...
  return {
    ...
    sections: {
      'olympiad-details': {
        ...
        type: 'paragraphs',
        image: { src: `/${meta.icon}`, alt: `Class ${classNum} ${subjName} Olympiad` },
        content: [
      ...
      benefits: {
        ...
        content: [
          ...
          isSenior ? 'Strengthens preparation for board exams and competitive entrance tests' : 'Creates a strong foundation for future Olympiads and competitive examinations',
        ],
      },
      'why-participate': {
      ...
      'how-to-apply': {
        ...
        type: 'paragraphs',
        image: { src: '/nti_register_banner.png', alt: 'Register for NTI Olympiad Exams - Mathematics, Science, English, IT, Finance - 1st to 10th' },
        content: [
      ...
      syllabus: { heading: 'Syllabus', type: 'list', content: topics },
      'exam-dates-and-fees': {
      ...
      'cut-off-and-answer-keys': {
        ...
        type: 'paragraphs',
        content: ['Cut-off marks and answer keys will be published after the examination. Students can check their performance against the official answer keys on the NTI website.'],
      },
      results: {
        heading: 'Results',
        type: 'paragraphs',
        content: ['Results will be declared within 4-6 weeks after the examination. Students and schools can access detailed performance reports through the NTI portal.'],
      },
...
let content = fs.readFileSync('./src/data/syllabusData.js', 'utf8');

['science', 'finance'].forEach((subject) => {
  const subjectEntries = {};
  for (let i = 1; i <= 10; i++) {
    subjectEntries[`class-${i}`] = genClassData(subject, i);
  }
  ...
  subjectObjectString += `  },\n`; // Important: Include the comma at the end for object formatting

  // Better replacement logic: Look for `science: {` and replace up to `  },`
  const regex = new RegExp(`^${subject}: \\{[\\s\\S]*?^  \\},`, 'm');
  
  if (regex.test(content)) {
      content = content.replace(regex, `  ${subjectObjectString.trimEnd()}`);
      console.log(`Matched and replaced ${subject}!`);
  } else {
      console.log(`Failed to match ${subject}!`);
  }
});

fs.writeFileSync('./src/data/syllabusData.js', content, 'utf8');
