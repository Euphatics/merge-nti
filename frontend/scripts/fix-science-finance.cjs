...
// To use generateClassData, we actually need to redefine it since the other script doesn't export it.
const subjectsMeta = {
  science: {
    name: 'Science',
...
function processSyllabusFile() {
  ...
  ['science', 'finance'].forEach((subject) => {
    ...
    // Now simply replace the exact block. We'll find it manually to be safe.
    // The block is:
    // subject: {
    ...
    content = content.replace(blockToReplace, subjectObjectString);
  });

  fs.writeFileSync(filePath, content, 'utf8');
  console.log('Successfully updated syllabusData.js with Science and Finance content!');
}
